...
```{r}
ETH_malaria_data_SPDF <- SpatialPointsDataFrame(coords = 
...
If you have a local raster file (e.g. a .tif file), you can use the `raster` command to load the file into R. You can also load using a URL.
...
```{r}
# NOTE: this crashed R Studio on DataHub for me, but you can try it on your local R Studio on your computer
